...
export async function GET(request: Request) {
  try {
    ...
    nodes.forEach(node => {
      if (node.type === "habit" && node.extra) {
        const hId = node.extra.id;
        const checkinDays = dayProfiles.filter(p => p.hasCheckin);
        
        const habitVector = checkinDays.map(p => p.completedHabits.has(hId) ? 1 : 0);
        const toneVector = checkinDays.map(p => p.tone);
        const energyVector = checkinDays.map(p => p.energy);

        const rTone = getPearsonCorrelation(habitVector, toneVector);
        const rEnergy = getPearsonCorrelation(habitVector, energyVector);

        // Average tone on completed vs not completed days
        const completedDays = checkinDays.filter(p => p.completedHabits.has(hId));
        const notCompletedDays = checkinDays.filter(p => !p.completedHabits.has(hId));

        const avgToneCompleted = completedDays.length > 0 
          ? completedDays.reduce((acc, curr) => acc + curr.tone, 0) / completedDays.length 
          : null;
        const avgToneNotCompleted = notCompletedDays.length > 0 
          ? notCompletedDays.reduce((acc, curr) => acc + curr.tone, 0) / notCompletedDays.length 
          : null;

        node.extra.toneCorrelation = parseFloat(rTone.toFixed(3));
        node.extra.energyCorrelation = parseFloat(rEnergy.toFixed(3));
        node.extra.avgToneCompleted = avgToneCompleted !== null ? parseFloat(avgToneCompleted.toFixed(2)) : null;
        node.extra.avgToneNotCompleted = avgToneNotCompleted !== null ? parseFloat(avgToneNotCompleted.toFixed(2)) : null;
      }
      ...
      if (node.type === "theme" && node.extra) {
        ...
        const rTone = getPearsonCorrelation(themeVector, toneVector);
        const rEnergy = getPearsonCorrelation(themeVector, energyVector);
    ...
    return NextResponse.json({
      ...
      links,
      summary: {
        totalDays: dayProfiles.length,
        checkinCount: dayProfiles.filter(p => p.hasCheckin).length,
        habitLogsCount: habitLogs.length,
      }
    });

  } catch (error: any) {
    console.error("Error in /api/correlations:", error);
    return NextResponse.json(
      { error: error?.message || "Internal server error during correlation analysis." },
      { status: 500 }
    );
  }
}
